...
def _convert_message_to_dict1(message: BaseMessage) -> dict:
    if isinstance(message, ChatMessage):
        message_dict = {'sender_type': message.role, 'sender_name': '小明', 'text': message.content}
    elif isinstance(message, HumanMessage):
        message_dict = {'sender_type': 'user', 'sender_name': '小明', 'text': message.content}
    elif isinstance(message, AIMessage):
        message_dict = {'sender_type': 'assistant', 'sender_name': '小明', 'text': message.content}
        if 'function_call' in message.additional_kwargs:
            message_dict['function_call'] = message.additional_kwargs[
                'function_call']
    elif isinstance(message, SystemMessage):
        message_dict = {'sender_type': 'system', 'sender_name': '小明', 'text': message.content}
    elif isinstance(message, FunctionMessage):
        message_dict = {
...
class ChatMinimaxAIProrag(BaseChatModel):
    ...
    def _create_message_dicts(
        ...
        message_dicts = [_convert_message_to_dict1(m) for m in messages]
        alltext = message_dicts[0]['text'] + "\n" + "Question:" + message_dicts[1]['text']  + "\nAnswer:"
           
        message_dicts1 = [{'sender_type': 'user', 'sender_name': '小明', 'text':alltext}]
        print(message_dicts1)
                

        return message_dicts1, params
