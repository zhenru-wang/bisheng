...
from .types import ChatInput, ChatOutput, Choice, Message, Usage, ChoiceMinimax
...
class ChatCompletion(object):
    ...
    def __init__(self, group_id, api_key, **kwargs):
        ep_url = 'https://api.minimax.chat/v1/text/chatcompletion_pro'
    ...
    def __call__(self, inp: ChatInput, verbose=False):
        ...
        if messages[0].sender_type == 'system':
        ...
        for m in chat_messages:
            role = 'USER'
            if m.sender_type == 'system' or m.sender_type == 'assistant':
                role = 'BOT'

            new_messages.append({'sender_type': role, "sender_name": "小明", 'text': m.text})
        ...
        if status_code == 200:
            ...
            if info['base_resp']['status_code'] == 1027:
                info = {'created': 0, 'model': 'abab6-chat', 'reply': '', 
                        'choices': [{'finish_reason': 'stop', 
                                     'messages': [{'sender_type': 'BOT', 'sender_name': 'MM智能助理', 'text': ''}]}], 
            ...
            try:
                if info['base_resp']['status_code'] == 0:
                    ...
                    for s in info['choices']:
                        ...
                        msg = Message(role='assistant', content=s['messages'][0]['text'])
                        cho = ChoiceMinimax(message=msg,
