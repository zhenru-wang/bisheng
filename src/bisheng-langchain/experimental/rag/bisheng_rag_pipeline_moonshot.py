...
import time 
# from langchain.chat_mdoels import ChatWenxin
...
class BishengRagPipeline():
    ...
    def __init__(self, yaml_path) -> None:
        ...
        if llm_params['type'] == 'ChatOpenAI':
            llm_params.pop('type')
            # print(self.params['chat_llm']['key'],self.params['chat_llm']['url'])
            self.llm = llm_object(
                # openai_api_key=self.params['chat_llm']['key'],
                # openai_api_base=self.params['chat_llm']['url'],
                # http_client=httpx.Client(proxies=openai_proxy),
                **llm_params
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--params', type=str, default='config/baseline_moonshot.yaml', help='bisheng rag params')
    # 解析参数
